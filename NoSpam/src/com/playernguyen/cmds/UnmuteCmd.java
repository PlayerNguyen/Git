...
public class UnmuteCmd implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(sender.hasPermission("nospam.unmute") || sender.hasPermission("nospam.*")){
			if(args.length == 0){
				sender.sendMessage("Hay su dung: /unmute [Player]");
				return true;
			}
			...
			PlayerManager.getPlayerManager().setMutePlayer(target, false);
